...
interface RealHoursSummaryModalProps {
  isOpen: boolean;
  onClose: () => void;
}
...
export default function RealHoursSummaryModal({ isOpen, onClose }: RealHoursSummaryModalProps) {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const total = calculateRealHours(fetchedActivities);
      setRealHours(total);
      setShowResults(true);
    } catch (error) {
  ...
  const handlePrint = useReactToPrint({
    content: () => document.getElementById('real-hours-report'),
  });

  const handlePDF = async () => {
    const element = document.getElementById('real-hours-report');
    if (!element) return;

    const opt = {
      margin: 1,
      filename: `riepilogo-ore-effettive.pdf`,
  ...
  const handleExportExcel = () => {
    const exportData = activities.map(activity => ({
      ...
      'Ore Effettive': activity.hours,
      Descrizione: activity.description
    }));

    const ws = XLSX.utils.json_to_sheet(exportData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Ore Effettive");
    ...
    const totalRow = {
      'Data': 'TOTALE ORE EFFETTIVE',
      'Ore Effettive': realHours.toFixed(1)
    };
    ...
    XLSX.writeFile(wb, "riepilogo-ore-effettive.xlsx");
  };
  ...
  return (
    <Modal
      ...
      style={modalStyles}
      contentLabel="Riepilogo Ore Effettive"
    >
      <div className="flex flex-col h-full">
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-bold text-gray-900">Riepilogo Ore in Classe</h2>
            <button
              onClick={onClose}
            ...
            <div id="real-hours-report">
              <div className="flex justify-between items-start mb-6">
                ...
                <div className="text-right">
                  <h2 className="text-xl font-bold">Riepilogo Ore in Classe</h2>
                  <p className="text-gray-600 mt-2">Ore effettive di lezione: {realHours.toFixed(1)}</p>
                </div>
              </div>
